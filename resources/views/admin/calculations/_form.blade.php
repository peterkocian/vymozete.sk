<div class="form-group row">
    ...
    <div class="col-sm-9">
        <input class="form-control {{ $errors->has('date') ? 'is-invalid' : '' }}" type="date" name="date" id="date" value="{{ old('date', \Carbon\Carbon::parse($data->date)->format('Y-m-d') ?? null) }}" required/>
        @error('date')
...
<div class="form-group row">
    ...
    <div class="col-sm-9">
        <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', $data->amount ?? null) }}" required/>
        @error('amount')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
<div class="form-group row">
    ...
    <div class="col-sm-9">
        <input class="form-control {{ $errors->has('paid') ? 'is-invalid' : '' }}" type="checkbox" name="paid" id="paid" value="{{ old('paid', $data->paid ?? null) }}" @if(old('paid', $data->paid ?? null) === 1) checked @endif/>
        @error('paid')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
